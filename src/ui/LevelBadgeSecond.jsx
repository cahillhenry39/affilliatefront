...
const StyledH5 = styled.h5`
  ...
  top: 50%;
  right: 50%;
  transform: translate(${50 * 0}%, ${-50 * 1.4}%);
  font-weight: 500;

  font-size: ${({ $size }) => `${$size}rem` || "0rem"};
  font-family: "Merriweather", serif;
`;

function LevelBadgeSecond({ level = 0, width = 3, levelColors }) {
  return (
    <StyledGetCurrency
    ...
    >
      ...
      <StyledH5 $size={(width / 5) * 1.5}>{level}</StyledH5>
    </StyledGetCurrency>
  );
}
...
export default LevelBadgeSecond;
